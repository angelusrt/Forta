...
function Login(props) {
    ...
    //Animation variables
    const heightAnim = useSharedValue(!animatable ? 330 : 450)
    const marginAnim = useSharedValue(!animatable ? 0 : 20)
    const opacAnim = useSharedValue(!animatable ? 1 : 0.3)
    ...
    //Sets animatable to false
    const onFinishedTransIn = () => setAnimatable(false)
    ...
    const transIn = () => {
        ...
        opacAnim.value = withTiming(1, {
            easing: Easing.elastic(1)
        }, isFinished => {if(isFinished) runOnJS(onFinishedTransIn)()})
    }
    ...
    //Goes to transIn when returns from "Register"
    useEffect(() => { if(animatable) transIn() },[animatable])

    useEffect(() => {
        if(route.params.prevScreen === "Login") {
            setAnimatable(true)
...
function Register(props) {
    ...
    //Animation variables
    const heightAnim = useSharedValue(!animatable ? 470 : 330)
    const marginAnim = useSharedValue(!animatable ? 0 : 120)
    const opacAnim = useSharedValue(!animatable ? 0.3 : 1)
    ...
    const onFinishedTransOut = () => {
        navigation.navigate("Login", {prevScreen: "Login"})
    }
    ...
    const onFinishedTransOutUser = () => {
        navigation.navigate("User", {email, password})
    }
    ...
    useEffect(() => { !animatable ? transIn() : transInUser() },[animatable])
...
function User(props) {
    ...
    //Animation variables
    const posAnim = useSharedValue(!animatable ? -110 : 90)
    const opacAnim = useSharedValue(!animatable ? 0.3 : 1)
